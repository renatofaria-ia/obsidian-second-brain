...
BOOTSTRAP_SKIP_DIRS = {".obsidian", ".git", "_export"}
...
def render_root_log(owner: str, preset_key: str, mode: str) -> str:
    return f"""---
type: log
---
...
## [{TODAY}] init | Bootstrapped bundle
- owner: {owner}
- preset: {preset_key}
- mode: {mode}
- files: index.md, log.md, _CLAUDE.md, Home.md
"""


def render_root_index(vault: Path, bundle_title: str) -> str:
    grouped: dict[str, list[Path]] = {}
    for path in sorted(vault.rglob("*.md")):
        rel = path.relative_to(vault)
        if any(part in BOOTSTRAP_SKIP_DIRS for part in rel.parts[:-1]):
            continue
        if rel.name in RESERVED_FILENAMES:
            continue
        section = rel.parts[0] if len(rel.parts) > 1 else "."
        grouped.setdefault(section, []).append(rel)
    ...
    lines = [
        ...
        f'# {bundle_title}',
        '',
        'Root index generated by bootstrap_vault.py.',
        '',
    ]

    root_entries = grouped.pop('.', [])
    if root_entries:
        lines.extend(['## Root', ''])
...
def bootstrap(vault: Path, name: str, preset_key: str, mode: str, subject: str,
    ...
    # ── .obsidian stub ────────────────────────────────────────────────────────
    bundle_owner = subject if mode == "assistant" else name
    write(vault / "log.md", render_root_log(bundle_owner, preset_key, mode))
    write(vault / "index.md", render_root_index(vault, f"{bundle_owner}'s Bundle"))
    print("Root index.md and log.md created")
    ...
    print("\n🧠 _CLAUDE.md is ready as an extension file, and the bundle root now includes canonical index.md and log.md.")
