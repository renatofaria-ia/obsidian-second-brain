...
triggers_pt: ["pesquisa profunda", "pesquisa completa", "pesquisa com base no bundle", "lacunas de pesquisa"]
---
...
4. **Free mode** - the script does Phase 1 (vault scan) plus free-source aggregation and prints a JSON block with `"mode": "free-sources-deep"`, containing `vault_baseline_notes` (path, score, excerpt of what the vault already knew), `sources` (fresh external results), `stats`, `warnings`, and an `instruction`. YOU are the synthesizer:
   ...
   - Produce a delta with exactly these sections: What's New Since Vault Baseline, What's Confirmed, Contradictions / Updates Needed (name the vault path or note reference), Synthesis, Recommended Vault Updates, Open Questions. Every external claim carries a recency marker and source domain; every vault reference uses the bundle's canonical internal-link format. Never invent facts to fill a section.
...
5. **Propagation (both modes):**
   ...
   - Then report back a clean list - which existing notes were updated, which new notes were created, and what was linked from today's daily note - using the real resolved paths, and note any synthesis bullet you could not ground (so a skipped update is visible, never silently dropped).
