...
The optional argument is a note path, an internal note reference, a folder, or a source URL/file. If none is given, ask what to distill (offer the longest recently-touched notes in `raw/` and the research folders as candidates).
...
1. Read `index.md` first if it exists in the bundle root. If `_CLAUDE.md` exists, treat it as an extension file that may refine local conventions. Resolve the destination folder per `references/folder-map.md` (a distillation is a concept/reference note: wiki-style `wiki/concepts/`, Obsidian-style `Knowledge/`).

2. Resolve and read the source:
   - A vault note or internal note reference: read it in full.
...
8. Link the distillation from the original source note (add a `Distilled:` line linking to the new note with the bundle's canonical internal-link format) and from today's daily note. Append a one-line entry to the operation log (`Logs/YYYY-MM-DD.md` if it exists, else `log.md`).
